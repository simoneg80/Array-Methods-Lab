const inventors = [
  ...
  const newArr = inventors.filter((inventor) => inventor.year > 1499 && inventor.year < 1600); 
    console.log(newArr); 
...
//   // Hint:  Return a new object literal from the callback (don't mutate the object being passed in to map)
  ...
  const nameProp = inventors.map((inventor) => inventor = `${inventor.first} ${inventor.last}`);
...
console.log(firstLast);
  ...
  const countInstances = data.reduce((acc, currentItem) => {
    acc[currentItem] = acc[currentItem] ? acc[currentItem] + 1 : 1;
    return acc
  }, {});
...
console.log(countInstances)
  ...
  const onePerson = devs.some((dev)=> {
    const currentYear = new Date().getFullYear();
    const yearValue = currentYear - dev.year >= 19;
    return yearValue;
  });
